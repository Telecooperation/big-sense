...
!function(factory) {
  ...
  vertx.EventBus = function(url, options) {
    ...
    that.login = function(username, password, sessionID, replyHandler) {
      sendOrPub("send", 'vertx.basicauthmanager.login', {username: username, password: password, sessionID: sessionID}, function(reply) {
       if (replyHandler) {
          replyHandler(reply)
        }
      });
    ...
    that.register = that.registerHandler = function(address, handler) {
    ...
    that.unregister = that.unregisterHandler = function(address, handler) {
      checkSpecified("address", 'string', address);
      checkSpecified("handler", 'function', handler);
      checkOpen();
      var handlers = handlerMap[address];
