...
import de.tudarmstadt.tk.dbsystel.wifimeasurement.localdb.MeasurementDB;
import de.tudarmstadt.tk.dbsystel.wifimeasurement.sensors.WifiNetworks;
import de.tudarmstadt.tk.dbsystel.wifimeasurement.sensors.PositionManager;
import de.tudarmstadt.tk.dbsystel.wifimeasurement.connection.HTTPFunctions;
import de.tudarmstadt.tk.dbsystel.wifimeasurement.sensors.WifiNetworks;
...
public class CollectAndSend {
    ...
    public CollectAndSend(Context context) {
        ...
        wifiNetworks = new WifiNetworks(context);
    ...
    public void makeMeasurement() {
        ...
        if(((StartService.ONLY_MEASURE_WHILE_MOVING && locationHasMoved) ||
                ...
                && wifiNetworks.getLastValuesForMeasurement() && location != null) {
            lastSavedLocation = location;

            List<Measurement> newNetworks = wifiNetworks.getMeasurements();
            for(Measurement measurement : newNetworks) {
                Date now = new Date();
                measurement.setDate(now.getTime());
                measurement.setLatitude(location.getLatitude());
                measurement.setLongitude(location.getLongitude());
                measurementDB.addMeasurement(measurement);
            }
            Log.i(StartService.LOG_TAG, "Stored Measurements");
        }
    }
